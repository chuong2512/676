...
public class SkillCard_S_K_0 : SkillCard
{
	private int realDmg;
	...
	public SkillCard_S_K_0(SkillCardAttr skillCardAttr)
		: base(skillCardAttr)
	{
	...
	private int RealDmg(Player player, out string atkDes)
	{
		...
		atkDes = BaseCard.GetRealDamageAtkDes(player, $"单倍武器伤害({player.PlayerAttr.AtkDmg})+当前信仰值({player.PlayerAttr.SpecialAttr})", IntData, out var pwdBuff, out var rate);
		return BaseCard.GetRealDamage(player, player.PlayerAttr.AtkDmg + player.PlayerAttr.SpecialAttr, IntData, pwdBuff, rate);
	}
	...
	private int RealDmg(Player player)
	{
		...
		return BaseCard.GetRealDamage(player, player.PlayerAttr.AtkDmg + player.PlayerAttr.SpecialAttr, IntData);
	}

	protected override string SkillOnBattleDes(Player player)
	{
		int num = RealDmg(player);
		int baseValue = player.PlayerAttr.AtkDmg + player.PlayerAttr.SpecialAttr;
		return string.Format(skillCardAttr.DesKeyOnBattle.LocalizeText(), player.PlayerAttr.AtkDmg, player.PlayerAttr.SpecialAttr, BaseCard.GetValueColor(baseValue, num), num);
	}
}
