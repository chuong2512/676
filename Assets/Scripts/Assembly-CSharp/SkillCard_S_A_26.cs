...
public class SkillCard_S_A_26 : SkillCard_Archer
{
	private int realDmg;
	...
	public SkillCard_S_A_26(SkillCardAttr skillCardAttr)
		: base(skillCardAttr)
	{
	...
	private void Effect(Player player, EnemyBase enTarget, Action handler)
	{
		...
		else
		{
			...
			if ((buff_Bleeding = Singleton<GameManager>.Instance.BattleSystem.BuffSystem.GetSpecificBuff(enTarget, BuffType.Buff_Bleeding) as Buff_Bleeding) != null)
			{
				enTarget.TakeDamage(buff_Bleeding.BuffRemainRound, null, isAbsDmg: true);
			}
		}
