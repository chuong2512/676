...
public class SkillCard_S_K_9 : SkillCard
{
	private int atkTime;
	...
	public SkillCard_S_K_9(SkillCardAttr skillCardAttr)
		: base(skillCardAttr)
	{
	...
	public override void SkillCardEffect(Player player, Action handler)
	{
		...
		atkTime = 3;
		Buff_BrokenArmor buff_BrokenArmor = Singleton<GameManager>.Instance.BattleSystem.BuffSystem.GetSpecificBuff(enemyPlayerChoose, BuffType.Buff_BrokenArmor) as Buff_BrokenArmor;
		if (!buff_BrokenArmor.IsNull())
		{
			atkTime += buff_BrokenArmor.BuffRemainRound;
		}
	...
	private void Effect(Player player, EnemyBase entityBase, int i)
	{
		...
		{
			gameReportUI.AddGameReportContent($"{base.CardName}效果触发：对{entityBase.EntityName}造第{i + 1}次非真实的{realDmg}点伤害({atkDes})");
		}
	}
