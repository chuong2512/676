...
public class SkillCard_S_A_16 : SkillCard_Archer
{
	private int realDmg;
	...
	public SkillCard_S_A_16(SkillCardAttr skillCardAttr)
		: base(skillCardAttr)
	{
	...
	private int RealDmg(Player player, out string atkDes)
	{
		...
		atkDes = BaseCard.GetRealDamageAtkDes(player, $"三倍武器伤害({player.PlayerAttr.AtkDmg * 3})", IntData, out var pwdBuff, out var rate);
		return SkillCard_Archer.GetRealDamage_Arrow(player, player.PlayerAttr.AtkDmg * 3, IntData, pwdBuff, rate);
	}

	private int RealDmg(Player player)
	{
		player.PlayerUseASkillCardPowUp(this, null, out var IntData);
		return SkillCard_Archer.GetRealDamage_Arrow(player, player.PlayerAttr.AtkDmg * 3, IntData);
	}
