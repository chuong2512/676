	...
	public class SerializationNodeDataWriter : BaseDataWriter
	{
		...
		public override void BeginReferenceNode(string name, Type type, int id)
		{
			Nodes.Add(new SerializationNode
			{
				...
				Data = (((object)type != null) ? (id.ToString(CultureInfo.InvariantCulture) + "|" + base.Context.Binder.BindToName(type, base.Context.Config.DebugContext)) : id.ToString(CultureInfo.InvariantCulture))
			});
		...
		public override void BeginStructNode(string name, Type type)
		{
			Nodes.Add(new SerializationNode
			{
				Name = name,
				Entry = EntryType.StartOfNode,
				Data = (((object)type != null) ? base.Context.Binder.BindToName(type, base.Context.Config.DebugContext) : "")
			});
		...
		public override void WritePrimitiveArray<T>(T[] array)
		{
			...
			List<SerializationNode> list2 = Nodes;
			item = new SerializationNode
			{
				Name = string.Empty,
				Entry = EntryType.PrimitiveArray,
