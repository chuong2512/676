...
public class GameEvent_43 : BaseGameEvent
{
	public override string GameEventCode => "Event_43";
	...
	{
		EnemyHeapData specificEnemyHeap = DataManager.Instance.GetSpecificEnemyHeap("MHeap_101");
		Singleton<GameManager>.Instance.StartBattle(new BattleSystem.EventBattleHandler(specificEnemyHeap), Vector2.one * 0.5f);
		EventManager.RegisterEvent(EventEnum.E_BattleFinallyEnd, OnBattleEnd);
	...
	private void OnBattleEnd(EventData data)
	{
		...
		{
			BaseGameEvent.StartFollowEvent("Event_44", null);
			EventOver();
		}
		EventManager.UnregisterEvent(EventEnum.E_BattleFinallyEnd, OnBattleEnd);
	}
}
