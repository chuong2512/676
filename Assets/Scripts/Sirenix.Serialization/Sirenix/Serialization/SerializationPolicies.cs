					...
					lock (LOCK)
					{
						if (unityPolicy == null)
						{
							...
							unityPolicy = new CustomSerializationPolicy("OdinSerializerPolicies.Unity", allowNonSerializableTypes: true, delegate(MemberInfo member)
							{
								...
								return member.IsDefined<SerializeField>(inherit: false) || member.IsDefined<OdinSerializeAttribute>(inherit: false) || ((object)UnitySerializationUtility.SerializeReferenceAttributeType != null && member.IsDefined(UnitySerializationUtility.SerializeReferenceAttributeType, inherit: false));
							});
						}
					}
				}
