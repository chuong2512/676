...
public class SpecialUsualCard_BC_P_9 : SpecialUsualCard
{
	protected override PointDownHandler pointdownHandler { get; }

	protected override PointUpHandler pointupHandler { get; }

	public SpecialUsualCard_BC_P_9(UsualCardAttr usualCardAttr)
		: base(usualCardAttr)
	{
	...
	{
		player.GetBuff(new Buff_DamageRestrik(player, 2));
		player.GetBuff(new Buff_DeadPoison(player, 1));
		GameReportUI gameReportUI = SingletonDontDestroy<UIManager>.Instance.GetView("GameReportUI") as GameReportUI;
		if (gameReportUI != null)
		{
			gameReportUI.AddGameReportContent(base.CardName + "效果触发：自身获得2层反伤和1层致命毒药");
		}
		handler?.Invoke();
	}
}
