...
public class UIAnim_CardPresupposition : MonoBehaviour
{
	[SerializeField]
	...
	private float originClassBtnTransPosX;
	...
	public Image deleteBtn;

	public Image verifyBtn;

	public Transform classBtnTrans;
	...
	{
		Reset(mainHandSlots, supHandCardSlots, mainHandCards, supHandCards);
	...
	public void Init()
	{
		...
		originClassBtnTransPosX = classBtnTrans.transform.localPosition.x;
	}

	private void Reset(Dictionary<string, BagCardSlotCtrl> mainHandSlots, Dictionary<string, BagCardSlotCtrl> supHandCardSlots, Dictionary<string, BagCardCtrl> mainHandCards, Dictionary<string, BagCardCtrl> supHandCards)
	{
		...
		resetTxt.WithCol(0f);
		saveTxt.WithCol(0f);
		deleteBtn.WithCol(0f);
		verifyBtn.WithCol(0f);
		classBtnTrans.transform.localPosition = classBtnTrans.transform.localPosition.WithV3(originClassBtnTransPosX + 300f);
	...
	private IEnumerator StartAnimCo(Dictionary<string, BagCardSlotCtrl> mainHandSlots, Dictionary<string, BagCardSlotCtrl> supHandCardSlots, Dictionary<string, BagCardCtrl> mainHandCards, Dictionary<string, BagCardCtrl> supHandCards)
	{
		...
		tweenList.Add(quitTxt.DOFade(1f, 0.4f));
		tweenList.Add(inventoryCardRoot.DOFade(1f, 0.2f));
		yield return new WaitForSeconds(0.1f);
		tweenList.Add(mainCardScrollImg.DOFillAmount(1f, 0.3f));
		tweenList.Add(saveTxt.DOFade(1f, 0.4f));
		tweenList.Add(resetTxt.DOFade(1f, 0.4f));
		tweenList.Add(classBtnTrans.DOLocalMoveX(originClassBtnTransPosX, 0.5f).SetEase(Ease.InQuint));
		foreach (Transform button in buttonList)
		{
			tweenList.Add(button.DOLocalMoveY(originalButtonListPosY, 0.5f).SetEase(Ease.InQuint));
			yield return waitfor50mili;
		}
		tweenList.Add(deleteBtn.DOFade(1f, 0.5f));
		tweenList.Add(verifyBtn.DOFade(1f, 0.5f));
