	...
	public struct BoneMatrix
	{
		...
		private static BoneMatrix GetInheritedInternal(BoneData boneData, BoneMatrix parentMatrix)
		{
			...
			case TransformMode.Normal:
			{
				...
				float num16 = MathUtils.CosDeg(boneData.rotation + boneData.shearX) * boneData.scaleX;
				float num17 = MathUtils.CosDeg(degrees) * boneData.scaleY;
				float num18 = MathUtils.SinDeg(boneData.rotation + boneData.shearX) * boneData.scaleX;
				float num19 = MathUtils.SinDeg(degrees) * boneData.scaleY;
				result.a = num * num16 + num2 * num18;
				result.b = num * num17 + num2 * num19;
				result.c = num3 * num16 + num4 * num18;
				result.d = num3 * num17 + num4 * num19;
				break;
			}
			...
			{
				float num20 = num * num + num3 * num3;
				float num21;
				if (num20 > 0.0001f)
				{
					num20 = Math.Abs(num * num4 - num2 * num3) / num20;
					num2 = num3 * num20;
					num4 = num * num20;
					num21 = MathUtils.Atan2(num3, num) * (180f / (float)Math.PI);
				}
				else
				{
					num = 0f;
					num3 = 0f;
					num21 = 90f - MathUtils.Atan2(num4, num2) * (180f / (float)Math.PI);
				}
				float degrees2 = boneData.rotation + boneData.shearX - num21;
				float degrees3 = boneData.rotation + boneData.shearY - num21 + 90f;
				float num22 = MathUtils.CosDeg(degrees2) * boneData.scaleX;
				float num23 = MathUtils.CosDeg(degrees3) * boneData.scaleY;
				float num24 = MathUtils.SinDeg(degrees2) * boneData.scaleX;
