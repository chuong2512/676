...
public class UsualCard_BC_M_5 : UsualCard
{
	private int realDmg;
	...
	public UsualCard_BC_M_5(UsualCardAttr usualCardAttr)
		: base(usualCardAttr)
	{
	...
	private void Effect(Player player, EnemyBase enTarget, Action handler)
	{
		...
		{
			gameReportUI.AddGameReportContent($"{base.CardName}效果触发：对{enTarget.EntityName}造成1次三倍武器伤害的非真实的{realDmg}点伤害({atkDes})");
		}
		handler?.Invoke();
	}
	...
	private int RealDmg(Player player, out string atkDes)
	{
		...
		int num = player.PlayerAttr.AtkDmg * 3;
		atkDes = BaseCard.GetRealDamageAtkDes(player, $"三倍的武器伤害({num})", IntData, out var pwdBuff, out var rate);
		return BaseCard.GetRealDamage(player, num, IntData, pwdBuff, rate);
	}

	private int RealDmg(Player player)
	{
		player.PlayerUseAUsualCardPowUp(this, out var IntData);
		int baseDmg = player.PlayerAttr.AtkDmg * 3;
		return BaseCard.GetRealDamage(player, baseDmg, IntData);
	}

	public override string GetOnBattleDes(Player player, bool isMain)
	{
		int baseValue = player.PlayerAttr.AtkDmg * 3;
		int num = RealDmg(player);
		return string.Format(usualCardAttr.DesKeyOnBattle.LocalizeText(), player.PlayerAttr.AtkDmg, BaseCard.GetValueColor(baseValue, num), num);
	}
}
