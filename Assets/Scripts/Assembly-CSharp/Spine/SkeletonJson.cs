		...
		private void ReadAnimation(Dictionary<string, object> map, string name, SkeletonData skeletonData)
		{
			...
			if (map.ContainsKey("ik"))
			{
				foreach (KeyValuePair<string, object> item12 in (Dictionary<string, object>)map["ik"])
				{
					...
					{
						ikConstraintTimeline.SetFrame(num9, (float)item13["time"], GetFloat(item13, "mix", 1f), GetBoolean(item13, "bendPositive", defaultValue: true) ? 1 : (-1), GetBoolean(item13, "compress", defaultValue: true), GetBoolean(item13, "stretch", defaultValue: false));
