...
public class EnemyCtrl_52 : EnemyBaseCtrl
{
	[SerializeField]
	...
	protected override void InitComponent()
	{
		...
		EnemyAttr enemyAttr = new EnemyAttr(DataManager.Instance.GetEnemyAttr("Monster_52"));
		enemyEntity = new Enemy_52(enemyAttr, this);
		enemyAttr.SetEnemyBase(enemyEntity);
	}

	public void SetDragonHeadCtrl(Monster50AnimCtrl monster50AnimCtrl)
	{
		_dragonHandAnimCtrl.SetDragonHeadAnimCtrl(monster50AnimCtrl, isLeft: true);
	}
	...
	private IEnumerator Action4Anim_IE(Transform[] targets, int time, Action<int> effect, Action actionOver)
	{
		...
		{
			skeletonAnimName = "zuozhua_attack4",
			skeletonAnimTrack = 0,
			isLoop = false
		});
